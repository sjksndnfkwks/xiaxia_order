...
| 消息 | 云开发实时聊天，文字/表情/图片/视频/语音；输入框 textarea 自动换行，超出 7 行显示 ⛶ 全屏编辑按钮；按住说话浮层提供「松开发送」和「语音转文字」两个圆按钮，手指滑动切换（语音转文字依赖微信「同声传译」插件）；语音气泡用三道弧形波纹动画，宽度按时长动态变化；长按菜单：复制/引用/多选/删除；多选可合并成一条转发气泡，引用消息以灰底白字附在原气泡下方 |
...
| 我的 | 已登录：头像/昵称编辑、订单历史、想吃清单入口、公告入口、管理员入口、退出账号；未登录：默认灰头像 + "未登录"提示，点击头像或底部"登录账号"行弹出 ActionSheet（微信登录 / 测试账号）。退出后订单仍保留在云端，重新登录自动恢复 |
...
- 微信「同声传译」插件（`provider: wx069ba97219f66d99`，需在小程序后台 → 设置 → 第三方设置 → 插件管理添加；个人主体可能不可用，未启用时语音转文字按钮会提示需先开通）
