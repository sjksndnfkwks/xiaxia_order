...
| 消息 | 云开发实时聊天，文字/表情/图片/视频/语音；输入框 textarea 自动换行，超出 7 行显示 ⛶ 全屏编辑按钮；按住录音、松开发送；语音气泡用三道弧形波纹动画，宽度按时长动态变化；长按菜单：复制/引用/多选/删除；多选可合并成一条转发气泡，引用消息以灰底白字附在原气泡下方 |
| 管理员消息侧边栏 | 仅管理员可见。左上角 ☰ 展开侧边栏，按最近消息时间排序展示所有给管理员发过消息的用户（头像/昵称从 users 集合实时拉取，长按可改备注），点击切换到对应会话并加载历史 |
...
| 我的 | 已登录：头像/昵称编辑、订单历史、想吃清单入口、公告入口、管理员入口、退出账号；未登录：默认灰头像 + "未登录"提示，点击头像或底部"登录账号"行弹出 ActionSheet（默认仅"用微信账号登录"，1.5 秒内连点 5 次解锁"用测试账号登录"隐藏入口）。退出仅对当前会话生效，下次冷启动自动登录回微信号 |
| 登录策略 | 冷启动自动调 `login` 云函数取 openid。未登录用户在下单 / 发消息 / 添加想吃清单等写操作入口会被 `utils/auth-guard.js` 拦截，弹「需要登录」modal，确认后立即触发微信登录 |
...
> 所有图片/视频/语音文件统一上传到云存储（`wx.cloud.uploadFile`），数据库只存 `cloud://` fileID。本地代码包仅保留 tabBar 图标和默认头像/空状态占位图等 UI 资源。
